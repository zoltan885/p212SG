...
try:
    ...
    logging.warning('Could not import PyTango and HasyUtils')
...
def lsenvironment():
    '''
    convenience function to list the environment variables
    ...
    ip.run_line_magic('lsenv', '')


def _prepare_config_file(path):
    '''
    Convenience function to prepare a starting configuration file, so that only the correct motors need to be inserted.

    Parameters
    ----------
    path : sting
        path to prepare the configuration file
...
def _getFullName(dev):
    '''
    Gets the full name of a device, that may be defined locally (without its host)

    Parameters
    ----------
    dev : PyTango.DeviceProxy
        The device 

    Returns
    -------
    string
        The full name with host appended at the beginning

    '''
    if ':10000/' in dev.dev_name():
        return dev.dev_name()
...
class Measurement:
    ...
    def __init__(self, config_file, path, load=False):
        ...
        self.detChannels = {'1': 'Perkin', '2': 'Eiger', '3': 'Lambda'}  # todo get configuration from the abtgv2
    ...
    def import_device(self, dev_name):
        ...
        try:
            ...
            self.write_log('%s\nWarning! %s imported successfully, but does not respond.' % (e, dev_name))
        if DEBUG:
            print('%s device imported' % dev_name)
        if DEBUG:
            self.write_log('%s device imported' % dev_name)
        return dev
    ...
    def set_slit_size(self, posdict):
        ...
        for m, p in posdict.items():
            c = c + ('%s %.3f ' % (m, p))
        HU.execMacro(c)  # self.spock.magic(c)
...
class TestGrain(object):
    def __init__(self, name, logger):
        ...
        self.logAttrs = ['timestamp', 'action', 'direction', 'detector',
                         'slit', 'scanID', 'ROIs', 'integRes', 'fitpars', 'positions']
...
class Grain(object):
    ...
    '''
    all_grains = []  #
    all_instances = []  # Not sure that collecting the instances is useful

    def __init__(self, M, name, grdct=None, redef=False):
        self.name = name
        self.M = M
        ...
        if redef and name in Grain.all_grains:
            idx = Grain.all_grains.index(name)
            Grain.all_grains.remove(name)  # the object itself will get garbage collected (I hope)
            del Grain.all_instances[idx]
        assert not name in Grain.all_grains, 'Redefinition of grain only allowed with option redef=True'
        Grain.all_grains.append(name)
        ...
        if grdct is not None:
            ...
            self.integRes = [['?'],]
            self.fitpars = [{'?': '?'}]
            self.logAttrs = ['timestamp', 'direction', 'detector', 'slit',
                             'scanID', 'ROIs', 'integRes', 'fitpars', 'positions']
        ...
        if DEBUG:
            print(self.mot_hor, self.mot_ver, self.mot_rot, self.detmot_hor)  # , self.detmot_ver)
        self.spock = get_ipython()
...
#        self.logAttrs = ['timestamp', 'direction', 'detector', 'slit', 'scanID', 'ROIs', 'integRes', 'fitpars', 'positions']
    ...
    def centerO(self, start, end, NoSteps, exposure=DEFEXPTIME, channel=2, auto=False, mode=24, log=True):
        ...
        positions, res, self.cROIs[str(channel)], fio = _func.centerOmega(
            start, end, NoSteps, exposure=exposure, channel=channel, roi=self.cROIs[str(channel)])
    ...
    def recenter(self, imsource=None, channel=2, every=None):
        if imsource is not None:
            if False:  # testing
                ...
                res = _func.fitGauss(imsource, self.cROIs[str(channel)],
                                     motor=None, gotoButton=False, gotofitpos=False)
                return
            self.cROIs[str(channel)], _ = _func.explorer(imsource, ROI=None)
            self.M.write_log('%s roi redefined: %s for grain %s' %
                             (self.M.detChannels[str(channel)], str(self.cROIs[str(channel)]), self.name))
        else:
        ...
        res = _func.fitGauss(imsource, self.cROIs[str(channel)], motor=None,
                             gotoButton=False, gotofitpos=False, every=every)
